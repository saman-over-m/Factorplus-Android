...
const KEY='fpSaved'; const PAGE_SIZE=10;
let savedCache=[];
...
let items=JSON.parse(localStorage.getItem('fpDraftItems')||'null')||[{name:'کالای جدید',qty:1,price:0}];
...
function setDefaults(){let d=JSON.parse(localStorage.getItem('fpDraft')||'null');$('#date').value=d?.date||todayJalali();$('#invoiceNo').value=d?.invoiceNo||String(Math.floor(10000+Math.random()*89999));$('#customer').value=d?.customer||'';$('#phone').value=d?.phone||'';$('#companyName').value=d?.companyName||getCompany();$('#discount').value=d?.discount||0;$('#tax').value=d?.tax||0;$('#currencyUnit').value=d?.currency||unit();$('#pageMode').value=d?.pageMode||'next';$('#paperSize').value=d?.paperSize||'A4';if(d?.items?.length)items=d.items;accent=d?.accent||accent;syncAccentPicker();if(d?.template){template=d.template;saveTemplate()}renderItems();applyFont();renderLogoPreview();render();refreshAll();updateStatus();runWeeklyBackup()}
function renderItems(){let box=$('#items');box.innerHTML=items.map((x,i)=>`<div class="item" draggable="true" data-drag="${i}"><div class="drag-handle">⠿</div><label class="desc">شرح کالا/خدمت<input data-i="${i}" data-k="name" value="${esc(x.name)}"></label><label>تعداد<input type="number" min="0" step="any" data-i="${i}" data-k="qty" value="${x.qty}"></label><label>قیمت واحد<input type="number" min="0" step="1" data-i="${i}" data-k="price" value="${x.price}"></label><div class="move-actions"><button class="move" data-up="${i}">↑</button><button class="move" data-down="${i}">↓</button><button class="remove" data-remove="${i}">×</button></div></div>`).join('');
$$('#items input').forEach(el=>el.addEventListener('input',()=>{items[+el.dataset.i][el.dataset.k]=el.dataset.k==='name'?el.value:Number(el.value);persistDraft();markChanged();render()}));
$$('[data-remove]').forEach(b=>b.onclick=()=>{items.splice(+b.dataset.remove,1);if(!items.length)items=[{name:'کالای جدید',qty:1,price:0}];renderItems();persistDraft();markChanged();render()});
...
function addItem(){items.push({name:'کالای جدید',qty:1,price:0});renderItems();persistDraft();markChanged();render();requestAnimationFrame(()=>{let e=$('#items .item:last-child');e?.querySelector('input')?.focus()})}
function applyFont(){let v=localStorage.getItem('fpFont')||'Noto',custom=localStorage.getItem('fpCustomFont');document.documentElement.style.setProperty('--invoice-font',v==='custom'&&custom?'FP_Custom':v==='Tahoma'?'Tahoma':v==='Arial'?'Arial':'Noto');if(v==='custom'&&custom){let st=$('#customFontStyle')||document.createElement('style');st.id='customFontStyle';st.textContent=`@font-face{font-family:FP_Custom;src:url(${custom}) format('truetype');font-display:swap}`;if(!st.parentNode)document.head.appendChild(st)}}
...
function invoicePage(rows,p,totalPages){
  ...
  const blocks={
    ...
    table: show('table') ? `<section class="tpl-section" data-tpl="table"><table class="inv-table density-${template.density}"><thead><tr><th>ردیف</th><th>شرح کالا / خدمت</th><th>تعداد</th><th>قیمت واحد</th><th>مبلغ کل</th></tr></thead><tbody>${rows.map((x,k)=>`<tr><td>${fa((p-1)*PAGE_SIZE+k+1)}</td><td>${esc(x.name)}</td><td>${fa(x.qty)}</td><td>${money(x.price)}</td><td>${money((x.qty||0)*(x.price||0))}</td></tr>`).join('')}</tbody></table></section>` : '',
  ...
  return `<article class="invoice-page" style="--r:${template.radius}px"><div class="invoice-content">${ordered}</div></article>`;
}
function render(){
  let pages= $('#pageMode').value==='single'?1:Math.max(1,Math.ceil(items.length/PAGE_SIZE));
  let html='';
  for(let p=1;p<=pages;p++) html+=invoicePage($('#pageMode').value==='single'?items:items.slice((p-1)*PAGE_SIZE,p*PAGE_SIZE),p,pages);
...
function newInvoice(){editingId=null;localStorage.removeItem('fpEditingId');items=[{name:'کالای جدید',qty:1,price:0}];['customer','phone'].forEach(k=>$('#'+k).value='');$('#invoiceNo').value=String(Math.floor(10000+Math.random()*89999));$('#date').value=todayJalali();$('#discount').value=0;$('#tax').value=0;persistDraft();renderItems();render();updateStatus();showSection('invoice')}
function loadInvoice(id){let a=savedCache,x=a.find(v=>v.id===id);if(!x)return;editingId=x.id;localStorage.setItem('fpEditingId',id);items=x.items||[];['customer','phone','invoiceNo','date','companyName','discount','tax'].forEach(k=>{if($('#'+k)&&x[k]!=null)$('#'+k).value=x[k]});$('#currencyUnit').value=x.currency||'toman';$('#pageMode').value=x.pageMode||'next';$('#paperSize').value=x.paperSize||'A4';accent=x.accent||accent;syncAccentPicker();if(x.logo)localStorage.setItem('fpLogo',x.logo);else localStorage.removeItem('fpLogo');if(x.template){template=x.template;saveTemplate()}renderLogoPreview();renderItems();render();updateStatus('ثبت شده');showSection('invoice')}
function loadSaved(){let a=[...savedCache],q=($('#savedSearch')?.value||'').trim().toLowerCase();if(q)a=a.filter(x=>String(x.customer).toLowerCase().includes(q)||String(x.invoiceNo).includes(q));let s=$('#savedSort')?.value||'dateDesc';a.sort((x,y)=>s==='dateAsc'?jalaliKey(x.date)-jalaliKey(y.date):s==='noDesc'?Number(y.invoiceNo)-Number(x.invoiceNo):s==='noAsc'?Number(x.invoiceNo)-Number(y.invoiceNo):s==='customer'?String(x.customer).localeCompare(String(y.customer),'fa'):jalaliKey(y.date)-jalaliKey(x.date)||Number(y.invoiceNo)-Number(x.invoiceNo));$('#savedList').innerHTML=a.length?a.map(x=>`<div class="saved-row clickable"><span><b>فاکتور ${fa(x.invoiceNo)}</b> — ${esc(x.customer||'بدون نام')} — ${esc(x.date||'')}</span><button class="soft" data-load="${x.id}">باز کردن</button></div>`).join(''):'<div class="muted">هنوز فاکتوری ثبت نشده است.</div>';$$('[data-load]').forEach(b=>b.onclick=()=>loadInvoice(b.dataset.load))}
...
function invoiceSvgPage(pg,pageIndex,totalPages){
  const {w:W,h:H}=exportPageSize();
  const z=calc(); const a=accent||'#2b7cff'; const escSvg=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&apos;'}[m]));
  const c=$('#customer').value||'مشتری', phone=$('#phone').value||'—', no=$('#invoiceNo').value||'—', date=$('#date').value||'—', company=$('#companyName').value||getCompany();
  const rows=[...pg.querySelectorAll('tbody tr')].map(r=>[...r.children].map(x=>x.textContent.trim()));
  const right=W-42,left=42; let parts=[];
  parts.push(`<rect width="${W}" height="${H}" fill="#fff"/><rect width="${W}" height="7" fill="${a}"/>`);
  parts.push(`<g font-family="Tahoma,Arial,sans-serif" fill="#172033">`);
  parts.push(`<text x="${right}" y="58" text-anchor="end" font-size="27" font-weight="700">فاکتور فروش</text><text x="${right}" y="86" text-anchor="end" font-size="13" fill="#64748b">${escSvg(company)}</text>`);
  ...
  parts.push(`<rect x="${left}" y="118" width="${W-84}" height="82" rx="10" fill="#f5f8ff" stroke="#dbe3ef"/>`);
  parts.push(`<text x="${right-14}" y="145" text-anchor="end" font-size="13" font-weight="700">مشخصات مشتری</text><text x="${right-14}" y="168" text-anchor="end" font-size="12">${escSvg(c)}</text><text x="${right-14}" y="187" text-anchor="end" font-size="11" fill="#64748b">${escSvg(phone)}</text>`);
  parts.push(`<text x="${left+14}" y="145" text-anchor="start" font-size="13" font-weight="700">فروشنده</text><text x="${left+14}" y="168" text-anchor="start" font-size="12">${escSvg(company)}</text>`);
  let y=224; parts.push(`<rect x="${left}" y="${y}" width="${W-84}" height="34" rx="6" fill="${a}"/>`);
  ...
  rows.forEach((r,idx)=>{parts.push(`<g fill="#172033" font-size="10">${r.map((t,j)=>`<text x="${Math.min(xs[j]||W-55,W-55)}" y="${y}" text-anchor="${j<2?"end":j===2?"middle":"middle"}">${escSvg(t)}</text>`).join('')}</g><line x1="${left}" x2="${W-42}" y1="${y+13}" y2="${y+13}" stroke="#dfe4ea"/>`);y+=36;});
  if(pageIndex===totalPages){
    ...
    const ty=H-205;
    parts.push(`<rect x="${boxX}" y="${ty-20}" width="${boxW}" height="128" rx="10" fill="#f5f8ff" stroke="#dbe3ef"/>`);
...
function getExportSvgs(){const pages=[...document.querySelectorAll('#invoicePreview .invoice-page')];if(!pages.length)throw new Error('Invoice preview is empty');return pages.map((p,i)=>invoiceSvgPage(p,i+1,pages.length));}
function svgToPngUrl(svg,W,H){return new Promise((resolve,reject)=>{const blob=new Blob([svg],{type:'image/svg+xml;charset=utf-8'}),url=URL.createObjectURL(blob),img=new Image();img.onload=()=>{try{const c=document.createElement('canvas'),scale=2;c.width=W*scale;c.height=H*scale;const ctx=c.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,0,0,c.width,c.height);URL.revokeObjectURL(url);resolve(c.toDataURL('image/jpeg',.94))}catch(e){URL.revokeObjectURL(url);reject(e)}};img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error('SVG render failed'))};img.src=url;});}
...
async function buildExportImages(){const svgs=getExportSvgs(),{w,h}=exportPageSize(),out=[];for(const s of svgs)out.push(await svgToPngUrl(s,w,h));return out}
async function downloadPNG(){try{const imgs=await buildExportImages();const u=imgs[0];window.__lastPNG=u;$('#imageResult').innerHTML=`<img src="${u}" alt="پیش‌نمایش فاکتور">`;$('#imageModal').classList.remove('hidden')}catch(e){console.error('PNG export:',e);alert('ساخت تصویر فاکتور انجام نشد. لطفاً دوباره تلاش کنید.')}}
async function showPDF(){try{const imgs=await buildExportImages();const blob=makePdfFromImages(imgs,$('#paperSize').value);const old=window.__lastPDF;if(old)URL.revokeObjectURL(old);const url=URL.createObjectURL(blob);window.__lastPDF=url;if(window.AndroidBridge){const fr=new FileReader();fr.onload=()=>window.AndroidBridge.openPdf('factorplus-invoice.pdf',fr.result);fr.readAsDataURL(blob);return}$('#pdfFrame').src=url+'#zoom=page-fit';$('#pdfModal').classList.remove('hidden')}catch(e){console.error('PDF export:',e);alert('ساخت پیش‌نمایش PDF انجام نشد. لطفاً دوباره تلاش کنید.')}}
async function downloadBlob(url,name){if(window.AndroidBridge){try{const r=await fetch(url);const b=await r.blob();const fr=new FileReader();fr.onload=()=>window.AndroidBridge.saveBase64File(name,fr.result);fr.readAsDataURL(b);return}catch(e){console.error('Android save:',e)}}let a=document.createElement('a');a.href=url;a.download=name;a.click()}
